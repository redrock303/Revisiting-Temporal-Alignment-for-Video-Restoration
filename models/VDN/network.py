...
class HR_Align(nn.Module):
    def __init__(self, nf=64, groups=8):
        ...
        self.dcnpack1 = DCN_sep(nf, nf, 3, stride=1, padding=1, dilation=1,
    ...
    def forward(self,nbr_fea,ref_fea,pre_offset_fea=None):
        ...
        align_fea = self.lrelu(self.dcnpack1(nbr_fea, offset))
...
class easy_fuse(nn.Module):
    def __init__(self, nf=64, nframes=3, has_relu=True):
        ...
        self.fea_fusion1 = nn.Conv2d(nf * nframes, nf, 5, 1, 2, bias=True)
        self.lrelu = nn.LeakyReLU(negative_slope=0.1, inplace=False)

    def forward(self, aligned_fea):
        B, N, C, H, W = aligned_fea.size()
        aligned_fea = aligned_fea.view(B, N * C, H, W)
        out = self.fea_fusion1(aligned_fea)
...
class RTA_VDN(nn.Module):
    def __init__(self, cfg=None):
        super(RTA_VDN, self).__init__()
        ...
        self.blending_conv = nn.Conv2d(nf+1+3, nf , 3, 1, 1, bias=True)
    ...
    def forward(self, x,noise_map,gt = None):
        ...
        for i in range(self.nbr * 2 + 1):
            if i != self.nbr:
                nbr_l.append(self.fea_extract( torch.cat([x[:, i, ...].contiguous(),noise_map],1) ))
            else:
                ref_fea = self.fea_extract( torch.cat([x[:, i, ...].contiguous(),noise_map],1) )
        ...
        al_fea = self.localCorr([f_41_a,f_11_a,f_3_a,f_2_a],ref_fea)
    
        
        fuse_fea = self.fuse(al_fea)
        mr_fea = self.lrelu(self.ps(self.up_conv1(fuse_fea)))
        hr_fea = self.lrelu(self.ps(self.up_conv2(mr_fea)))
        

        fuse_fea = self.lrelu( self.blending_conv(torch.cat([hr_fea,base,noise_map],1)) )
        
        recon_fea = self.recon(fuse_fea)
        out = self.out_conv(self.lrelu(self.hr_conv(recon_fea)))
...
if __name__ == '__main__':
    ...
    ins = torch.rand((2, 5, 3, 64, 64))
    noise_map = torch.rand((2, 1, 64, 64)).to(device)
    ins = ins.to(device)

    with torch.no_grad():
        out = net(ins,noise_map)[0]
    print(out.shape)
