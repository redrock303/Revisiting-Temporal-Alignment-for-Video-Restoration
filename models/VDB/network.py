...
sys.path.append('/home/kunzhou/project/my_deblur/')
import torch
...
class ResidualBlock_noBN(nn.Module):
    def __init__(self, nf=64):
        ...
        self.conv1 = nn.Conv2d(nf, 96, 3, 1, 1, bias=True)
        self.conv2 = nn.Conv2d(96, nf, 3, 1, 1, bias=True)
...
class encoder(nn.Module):
    def __init__(self, nf=64, N_RB=5):
        ...
        self.conv_first01 =  torch.nn.Sequential(
            nn.Conv2d(3, nf*2, 3, 2, 1, bias=True),
...
class RTA_VDB(nn.Module):
    def __init__(self, cfg=None):
        super(RTA_VDB, self).__init__()
...
if __name__ == '__main__':
    ...
    net = RTA_VDB(cfg = config)
    net.to(device)
    print("RTA_VDB have {:.3f}M paramerters in total".format(sum(x.numel() for x in net.parameters())/1000000.0)) 

    ins = torch.randn(2,5,3,1280,640).to(device)
    with torch.no_grad():
        out = net(ins)
    print('out',out.shape)
